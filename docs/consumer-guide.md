...
固定引用额外提供 `version.content_url`：GitHub commit 与可选仓库内路径组成 `/tree/<完整 commit>/<逐段编码路径>`，可以精确打开对应文件或目录的位置；外部内容 URL 加校验值则保留该外部位置。`canonical_url` 仍表示来源仓库，`version.url` 仍表示原契约中的来源引用，二者不会被替换成固定文件链接。普通 Git 相对路径中的空格和非 ASCII 字符按路径段编码；只有可移动 ref 的引用不产生 `content_url`。提供链接不等于已请求该内容或验证其仍可访问。
