...
随后通过获准使用的 Chrome 原生界面成功加载正式 `https://aipoch.network/`。页面显示 **Not connected**，没有 Demo 标签；点击 **Connect to open: AnnData** 后，浮窗保留 AnnData，点击 **Connect Open-Science** 后出现 Chrome 的“访问此设备上的其他应用和服务”权限提示。实施任务已向用户请求批准，尚未点击允许。当前证据达到正式页面与浏览器权限边界，尚未证明配对和收件成功。隔离测试宿主和既有 Connector 经实际查询仍为 ready，没有另启重复服务。
